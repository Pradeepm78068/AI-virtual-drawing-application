...
drawing_image = np.zeros((720, 1280, 3), dtype=np.uint8)
...
def draw_color_palette(image):
    ...
    for i, color_name in enumerate(color_names):
        ...
        cv2.rectangle(image, (palette_x + i * (box_size + spacing), 10),
                      (palette_x + box_size + i * (box_size + spacing), 10 + box_size), color, -1)
